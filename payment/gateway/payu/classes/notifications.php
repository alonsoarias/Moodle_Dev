...
class notifications {
    /**
     * Function that handles the notifications about transactions using PayU payment gateway
     * and all kinds of responses.
     *
     * This function sends a message to the user and returns the ID of the message if needed
    ...
    public static function notify($userid, $fee, $currency, $orderid, $type = '') {
        ...
        $message->userto    = $user;
        $message->subject   = get_string('messagesubject', 'paygw_payu');
        
        // Select message based on type
        switch ($type) {
            case 'payment_completed':
                $messagebody = get_string('message_payment_completed', 'paygw_payu', $a);
                break;
