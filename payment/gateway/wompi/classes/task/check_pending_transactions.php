<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Scheduled task to check pending transactions status via Wompi API.
 *
 * This task is essential for async payment methods like PSE, Nequi, and cash payments
 * that may take minutes to hours to confirm.
 *
 * @package    paygw_wompi
 * @copyright  2025 ingeweb.co <soporte@ingeweb.co>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace paygw_wompi\task;

use core\task\scheduled_task;
use paygw_wompi\wompi_helper;
use paygw_wompi\gateway;
use core_payment\helper as payment_helper;

/**
 * Check pending transactions task.
 */
class check_pending_transactions extends scheduled_task {

    /**
     * Return the task's name as shown in admin screens.
     *
     * @return string
     */
    public function get_name(): string {
        return get_string('task_check_pending', 'paygw_wompi');
    }

    /**
     * Execute the task.
     */
    public function execute(): void {
        global $DB;

        mtrace('Starting Wompi pending transactions check...');

        // Get pending transactions from the last 48 hours (async methods can take time).
        $cutoff = time() - (48 * HOURSECS);
        $transactions = $DB->get_records_select(
            'paygw_wompi_transactions',
            "status = :status AND timecreated > :cutoff",
            ['status' => wompi_helper::STATUS_PENDING, 'cutoff' => $cutoff],
            'timecreated ASC',
            '*',
            0,
            100 // Process max 100 transactions per run.
        );

        if (empty($transactions)) {
            mtrace('No pending transactions found.');
            return;
        }

        mtrace('Found ' . count($transactions) . ' pending transactions to check.');

        $processed = 0;
        $updated = 0;
        $errors = 0;

        foreach ($transactions as $transaction) {
            $processed++;

            try {
                // Get the payment account config for this transaction.
                $config = $this->get_gateway_config($transaction);
                if (!$config) {
                    mtrace("  Transaction {$transaction->transactionid}: No gateway config found, skipping.");
                    continue;
                }

                // Create helper instance.
                $helper = gateway::create_helper_from_config($config);

                // Query Wompi API for current status.
                $wompidata = $helper->get_transaction($transaction->transactionid);

                if (!$wompidata || !isset($wompidata['status'])) {
                    mtrace("  Transaction {$transaction->transactionid}: Could not fetch status from Wompi.");
                    $errors++;
                    continue;
                }

                $newstatus = $wompidata['status'];
                mtrace("  Transaction {$transaction->transactionid}: Wompi status = {$newstatus}");

                // If status changed from PENDING, update the transaction.
                if ($newstatus !== wompi_helper::STATUS_PENDING) {
                    $this->update_transaction_status($transaction, $newstatus, $wompidata);
                    $updated++;
                }

            } catch (\Exception $e) {
                mtrace("  Transaction {$transaction->transactionid}: Error - " . $e->getMessage());
                $errors++;
            }
        }

        mtrace("Completed: Processed={$processed}, Updated={$updated}, Errors={$errors}");
    }

    /**
     * Get gateway configuration for a transaction.
     *
     * @param object $transaction The transaction record.
     * @return object|null The gateway config or null if not found.
     */
    private function get_gateway_config(object $transaction): ?object {
        global $DB;

        try {
            // Get the payment record.
            $payment = $DB->get_record('payments', [
                'component' => $transaction->component,
                'paymentarea' => $transaction->paymentarea,
                'itemid' => $transaction->itemid,
                'userid' => $transaction->userid,
            ]);

            if (!$payment) {
                return null;
            }

            // Get the gateway config.
            $config = (object) payment_helper::get_gateway_configuration(
                $transaction->component,
                $transaction->paymentarea,
                $transaction->itemid,
                'wompi'
            );

            return $config;
        } catch (\Exception $e) {
            return null;
        }
    }

    /**
     * Update transaction status and deliver order if approved.
     *
     * @param object $transaction The transaction record.
     * @param string $newstatus The new status from Wompi.
     * @param array $wompidata The full Wompi transaction data.
     */
    private function update_transaction_status(object $transaction, string $newstatus, array $wompidata): void {
        global $DB;

        // Update the transaction status.
        $transaction->status = $newstatus;
        $transaction->timemodified = time();
        $DB->update_record('paygw_wompi_transactions', $transaction);

        mtrace("    Updated status to: {$newstatus}");

        // If approved, deliver the order.
        if ($newstatus === wompi_helper::STATUS_APPROVED) {
            $this->deliver_order($transaction);
        }

        // Send notification to user.
        $this->send_user_notification($transaction, $newstatus);
    }

    /**
     * Deliver the order to the user.
     *
     * @param object $transaction The transaction record.
     */
    private function deliver_order(object $transaction): void {
        global $DB;

        // Check if already delivered.
        if ($transaction->status === 'DELIVERED') {
            mtrace("    Order already delivered, skipping.");
            return;
        }

        try {
            $payable = payment_helper::get_payable(
                $transaction->component,
                $transaction->paymentarea,
                $transaction->itemid
            );

            $cost = payment_helper::get_rounded_cost(
                $payable->get_amount(),
                $payable->get_currency(),
                payment_helper::get_gateway_surcharge('wompi')
            );

            // Record the payment.
            $paymentid = payment_helper::save_payment(
                $payable->get_account_id(),
                $transaction->component,
                $transaction->paymentarea,
                $transaction->itemid,
                $transaction->userid,
                $cost,
                $payable->get_currency(),
                'wompi'
            );

            // Deliver the order.
            payment_helper::deliver_order(
                $transaction->component,
                $transaction->paymentarea,
                $transaction->itemid,
                $paymentid,
                $transaction->userid
            );

            // Mark as delivered.
            $transaction->status = 'DELIVERED';
            $transaction->timemodified = time();
            $DB->update_record('paygw_wompi_transactions', $transaction);

            mtrace("    Order delivered successfully (payment ID: {$paymentid})");

        } catch (\Exception $e) {
            mtrace("    Failed to deliver order: " . $e->getMessage());
        }
    }

    /**
     * Send notification to user about payment status.
     *
     * @param object $transaction The transaction record.
     * @param string $status The payment status.
     */
    private function send_user_notification(object $transaction, string $status): void {
        global $DB;

        $user = $DB->get_record('user', ['id' => $transaction->userid]);
        if (!$user) {
            return;
        }

        $stringkey = 'payment_' . strtolower($status);
        $subject = get_string('payment_notification', 'paygw_wompi');
        $message = get_string_manager()->string_exists($stringkey, 'paygw_wompi')
            ? get_string($stringkey, 'paygw_wompi')
            : "Your payment status has been updated to: {$status}";

        $eventdata = new \core\message\message();
        $eventdata->component = 'paygw_wompi';
        $eventdata->name = 'payment_status';
        $eventdata->userfrom = \core_user::get_noreply_user();
        $eventdata->userto = $user;
        $eventdata->subject = $subject;
        $eventdata->fullmessage = $message;
        $eventdata->fullmessageformat = FORMAT_PLAIN;
        $eventdata->fullmessagehtml = '<p>' . $message . '</p>';
        $eventdata->smallmessage = $message;
        $eventdata->notification = 1;

        message_send($eventdata);
    }
}
