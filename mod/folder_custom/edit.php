...
/**
 * Manage files in folder_custom module instance
 *
...
require_capability('mod/folder_custom:managefiles', $context);
...
file_prepare_standard_filemanager($data, 'files', $options, $context, 'mod_folder_custom', 'content', 0);

$mform = new mod_folder_custom_edit_form(null, array('data'=>$data, 'options'=>$options));
...
if ($mform->is_cancelled()) {
    ...
    $formdata = file_postupdate_standard_filemanager($formdata, 'files', $options, $context, 'mod_folder_custom', 'content', 0);
    $folder_custom->timemodified = time();
    $folder_custom->revision = $folder_custom->revision + 1;

    $DB->update_record('folder_custom', $folder_custom);
...
echo $OUTPUT->header();
echo $OUTPUT->box_start('generalbox folder_customtree');
$mform->display();
echo $OUTPUT->box_end();
echo $OUTPUT->footer();
