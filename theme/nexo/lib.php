<?php
/**
 * Lib functions for theme_nexo
 *
 * @package   theme_nexo
 * @copyright (c) 2025 Vicerrectoría Académica - ISER <alonsoariasb@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @author    Alonso Arias (contratista del ISER) <alonsoariasb@gmail.com>
 * @developer Alonso Arias (contratista del ISER)
 * @developedfor Instituto Superior de Educación Rural (ISER)
 */
 ...
 * @return string
 */
function theme_nexo_get_extra_scss($theme) {
    ...
    if (file_exists(__DIR__ . '/scss/nexo.scss')) {
        $scss .= file_get_contents(__DIR__ . '/scss/nexo.scss');
    }
 ...
 * @return string
 */
function theme_nexo_get_pre_scss($theme)
{
    // Activar licencia cuando se genera el CSS
    theme_nexo_license_autoload();
 ...
 * @return string
 */
function theme_nexo_get_main_scss_content($theme) {
    global $CFG;
    ...
    // Luego, cargar las personalizaciones SCSS de nexo.
    $nexoVariables = '';
    $customVariables = '';
    $nexoScss = '';

    if (file_exists($CFG->dirroot . '/theme/nexo/scss/_variables.scss')) {
        $nexoVariables = file_get_contents($CFG->dirroot . '/theme/nexo/scss/_variables.scss');
    }
    if (file_exists($CFG->dirroot . '/theme/nexo/scss/custom_variables.scss')) {
        $customVariables = file_get_contents($CFG->dirroot . '/theme/nexo/scss/custom_variables.scss');
    }
    if (file_exists($CFG->dirroot . '/theme/nexo/scss/nexo.scss')) {
        $nexoScss = file_get_contents($CFG->dirroot . '/theme/nexo/scss/nexo.scss');
    }

    // Cargar cualquier CSS personalizado desde 'custom.css'.
    $customCss = '';
    if (file_exists($CFG->dirroot . '/theme/nexo/style/custom.css')) {
        $customCss = file_get_contents($CFG->dirroot . '/theme/nexo/style/custom.css');
    }
 ...
 * @return mixed
 */
function theme_nexo_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array())
{
    // Activar licencia cuando se sirven archivos
    theme_nexo_license_autoload();
    
    $theme = theme_config::load('nexo');
 ...
 */
function theme_nexo_before_footer() {
 ...
 */
function theme_nexo_process_css($css) {
 ...
 * @return void
 */
function theme_nexo_migrate_settings() {
    $oldtonew = [
    ...
    foreach ($oldtonew as $old => $new) {
        $value = get_config('theme_nexo', $old);
        if ($value !== false) {
            set_config($new, $value, 'theme_nexo');
            // Opcional: eliminar el valor antiguo
            // unset_config($old, 'theme_nexo');
        }
    }
}
