<?php
/**
 * Provides the core rendering functionality for the theme_nexo, aligning Moodle's HTML with Bootstrap expectations.
 *
 * This core renderer class extends theme_remui's core renderer, adding specific modifications to enhance and customize
 * the user interface for theme_nexo. Key functionalities include customized login forms, theme settings integration,
...
class core_renderer extends \theme_remui\output\core_renderer
{
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        // Logo from 'remui' or from 'nexo'? 
        // Si deseas usar el logo del tema "nexo" en vez de "remui", cambia la línea:
    ...
    /**
     * Devuelve la URL del logo. Puede cambiarse para usar 'nexo' o 'remui' según tu preferencia.
     *
     * @param string $img
     * @return string|null
     */
    ...
    public function standard_footer_html() {
        ...
        // Carga la configuración del tema nexo
        $theme = theme_config::load('nexo');
    ...
    public function full_header() {
        ...
        if (is_siteadmin() && (!empty($theme->settings->generalnoticemode) && $theme->settings->generalnoticemode === 'off')) {
            $output .= '<div class="alert mt-4"><a href="' . $CFG->wwwroot . '/admin/settings.php?section=themesettingnexo#theme_nexo">' .
                       '<strong><i class="fa fa-edit"></i></strong> ' . get_string('generalnotice_create', 'theme_nexo') . '</a></div>';
        }
...
/**
    ...
    protected function add_copy_paste_prevention() {
        ...
        try {
            ...
            if ($hasrestrictedrole) {
                ...
                $PAGE->requires->js_call_amd('theme_nexo/prevent_copy_paste', 'init');
    ...
    protected function show_unauthorized_access_overlay($popup_id) {
        ...
        $output .= '<h2 style="color: red;">' . get_string('unauthorized_access', 'theme_nexo') . '</h2>';
        $output .= '<p>' . get_string('unauthorized_access_msg', 'theme_nexo') . '</p>';
        $output .= '</div>';
        $output .= '</div>';
        ...
        $output .= '<script type="text/javascript">
            document.addEventListener("keydown", function(event) {
                ...
                if (event.keyCode == 123 || (event.ctrlKey && event.shiftKey && (event.keyCode == 73 || event.keyCode == 74))) {
                    ...
                    alert("' . get_string('devtools_access_disabled', 'theme_nexo') . '");
                    return false;
                }
            });
    ...
    protected function check_allowed_urls() {
        global $CFG;
        $allowed_urls = [
            'https://nexo.moodlesoporte.net',
            'http://nexo.moodlesoporte.net',
            'https://aulavirtual.nexo.edu.co',
            'http://aulavirtual.nexo.edu.co',
