...
class theme_nexo_admin_settingspage_tabs extends theme_boost_admin_settingspage_tabs
{
    ...
    public function set_tabs($tabs, $reset = true)
    {
        ...
        if (!empty($tabs)) {
            foreach ($tabs as $tab) {
                ...
                $tab->name = str_replace('theme_remui', 'theme_nexo', $tab->name);
                if (!empty($tab->settings)) {
                    foreach ($tab->settings as $setting) {
                        $original_setting = $setting->name;
                        $setting->name = str_replace('theme_remui', 'theme_nexo', $setting->name);
