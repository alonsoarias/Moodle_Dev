...
/**
 * Get my overview courses - Enhanced for theme_inteb
 *
 * This trait extends theme_remui's get_myoverviewcourses to include:
...
trait get_myoverviewcourses {
    ...
    /**
     * Get courses matching the given timeline classification - ENHANCED for theme_inteb
     *
     * This method extends RemUI's version to include:
     * 1. RemUI custom fields (edwcourseduration, edwskilllevel)
     * 2. All instructors (both editingteacher and teacher roles)
     *
    ...
    public static function get_myoverviewcourses(
        ...
        // Use inteb's coursehandler to get ALL instructors
        $coursehandler = new \theme_inteb_coursehandler();

        foreach ($formattedcourses as $formatecourse) {
            $completioninfo = new \completion_info($formatecourse);
            ...
            if ($allactivites < 1) {
                ...
                $activitydata = get_string('activitydata', 'theme_remui', [
                    'complete' => $completedactivites,
                    'total' => $allactivites
                ]);
            }
            ...
            // ========================================
            // INTEB ENHANCEMENT 1: Get ALL instructors
            // ========================================
            $allteachers = $coursehandler->get_all_course_teachers($formatecourse->id);
            $formatecourse->instructor = "";
            $formatecourse->instructorcount = "";

            if (!empty($allteachers)) {
                $teachercount = count($allteachers);
                $firstteacher = reset($allteachers);

                if ($firstteacher) {
                    $pictureurl = utility::get_user_picture($DB->get_record('user', array('id' => $firstteacher->id)));
                    $formatecourse->instructor = json_encode(array(
                        'name' => fullname($firstteacher, true),
                        'url'  => $CFG->wwwroot.'/user/profile.php?id='.$firstteacher->id,
                        'picture' => $pictureurl->__toString()
                    ));
                }

                $formatecourse->instructorcount = ($teachercount > 1) ? $teachercount - 1 : "";
            }

            // ========================================
            // INTEB ENHANCEMENT 2: Add RemUI custom fields
            // ========================================
            $customfields = get_all_remui_course_metadata($formatecourse->id);

            // Add course duration
            if (isset($customfields['edwcourseduration']) && !empty($customfields['edwcourseduration']['text'])) {
                $formatecourse->courseduration = $customfields['edwcourseduration']['text'];
                $formatecourse->hascourseduration = true;
            } else {
                $formatecourse->courseduration = '';
                $formatecourse->hascourseduration = false;
            }

            // Add skill level
            if (isset($customfields['edwskilllevel']) && !empty($customfields['edwskilllevel']['text'])) {
                $formatecourse->courseskilllevel = $customfields['edwskilllevel']['text'];
                $formatecourse->hascourseskilllevel = true;
            } else {
                $formatecourse->courseskilllevel = '';
                $formatecourse->hascourseskilllevel = false;
            }
    ...
    /**
     * Returns description of method result value - ENHANCED for theme_inteb
     *
    ...
    public static function get_myoverviewcourses_returns() {
        ...
        $testvar->keys['instructorcount'] = new external_value(PARAM_RAW, 'Instructor count', VALUE_OPTIONAL);
        $testvar->keys['activitydata'] = new external_value(PARAM_RAW, 'Activity data', VALUE_OPTIONAL);

        // INTEB ENHANCEMENTS: Add custom fields to return structure
        $testvar->keys['courseduration'] = new external_value(PARAM_TEXT, 'Course duration from custom field', VALUE_OPTIONAL);
        $testvar->keys['hascourseduration'] = new external_value(PARAM_BOOL, 'Has course duration', VALUE_OPTIONAL);
        $testvar->keys['courseskilllevel'] = new external_value(PARAM_TEXT, 'Course skill level from custom field', VALUE_OPTIONAL);
        $testvar->keys['hascourseskilllevel'] = new external_value(PARAM_BOOL, 'Has skill level', VALUE_OPTIONAL);
