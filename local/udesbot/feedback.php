...
try {
    ...
    if (!$rule) {
        echo json_encode([
            'success' => false,
            'error' => get_string('rulenotfound', 'local_udesbot')
        ]);
        die();
    }
    ...
    if ($recentfeedback) {
        ...
        $DB->update_record('local_udesbot_feedback', $recentfeedback);
    } else {
        // Insert new feedback.
        ...
        $DB->insert_record('local_udesbot_feedback', $feedback);
    }
    ...
    echo json_encode([
        'success' => true,
        'message' => get_string('feedback_thanks', 'local_udesbot')
    ]);
