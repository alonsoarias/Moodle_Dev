...
// Get all data.
$categories = $DB->get_records('local_udesbot_category', null, 'sortorder ASC');
$rules = $DB->get_records('local_udesbot_rule', null, 'categoryid ASC, priority DESC, timecreated ASC');
$options = $DB->get_records('local_udesbot_option', null, 'ruleid ASC, sortorder ASC');
...
if ($format === 'csv') {
    ...
    $filename = 'udesbot_rules_' . date('Y-m-d_His') . '.csv';
    ...
    $filename = 'udesbot_export_' . date('Y-m-d_His') . '.json';
