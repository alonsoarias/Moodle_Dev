...
class option_form extends \moodleform {
    ...
    protected function definition() {
        ...
        // Button text field.
        $mform->addElement('text', 'text', get_string('optiontext', 'local_udesbot'), ['size' => 50, 'maxlength' => 100]);
        $mform->setType('text', PARAM_TEXT);
        $mform->addRule('text', get_string('required'), 'required', null, 'client');
        $mform->addRule('text', get_string('maximumchars', '', 100), 'maxlength', 100, 'client');
        $mform->addHelpButton('text', 'optiontext', 'local_udesbot');

        // Icon field (emoji).
        $mform->addElement('text', 'icon', get_string('icon', 'local_udesbot'), ['size' => 10, 'maxlength' => 50]);
        $mform->setType('icon', PARAM_TEXT);
        $mform->addHelpButton('icon', 'icon', 'local_udesbot');

        // Target rule select.
        $rules = $DB->get_records_menu('local_udesbot_rule', ['enabled' => 1], 'pattern ASC', 'id, pattern');
        $ruleoptions = ['' => get_string('selecttargetrule', 'local_udesbot')] + $rules;
        $mform->addElement('select', 'targetruleid', get_string('targetrule', 'local_udesbot'), $ruleoptions);
        $mform->setType('targetruleid', PARAM_INT);
        $mform->addRule('targetruleid', get_string('required'), 'required', null, 'client');
        $mform->addHelpButton('targetruleid', 'targetrule', 'local_udesbot');

        // Sort order field.
        $mform->addElement('text', 'sortorder', get_string('optionorder', 'local_udesbot'), ['size' => 5]);
