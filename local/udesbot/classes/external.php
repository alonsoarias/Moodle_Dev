...
/**
 * External API for local_udesbot (v1.9.0).
 *
...
if (class_exists('\core_external\external_api')) {
    ...
    class_alias('\core_external\external_api', '\local_udesbot\base_external_api');
    class_alias('\core_external\external_function_parameters', '\local_udesbot\base_external_function_parameters');
    class_alias('\core_external\external_value', '\local_udesbot\base_external_value');
    class_alias('\core_external\external_single_structure', '\local_udesbot\base_external_single_structure');
    class_alias('\core_external\external_multiple_structure', '\local_udesbot\base_external_multiple_structure');
} else {
    // Legacy support for Moodle < 4.2.
    require_once($CFG->libdir . '/externallib.php');
    class_alias('\external_api', '\local_udesbot\base_external_api');
    class_alias('\external_function_parameters', '\local_udesbot\base_external_function_parameters');
    class_alias('\external_value', '\local_udesbot\base_external_value');
    class_alias('\external_single_structure', '\local_udesbot\base_external_single_structure');
    class_alias('\external_multiple_structure', '\local_udesbot\base_external_multiple_structure');
}

use context_system;

/**
 * External functions for udesbot mascot functionality.
 */
class external extends base_external_api {
    ...
    public static function get_popular_questions($limit = 5) {
        ...
        // Check capability.
        require_capability('local/udesbot:use', $context);
        ...
        $sql = "SELECT r.id, r.pattern, COUNT(l.id) as usage_count
                ...
                JOIN {local_udesbot_log} l ON l.ruleid = r.id
