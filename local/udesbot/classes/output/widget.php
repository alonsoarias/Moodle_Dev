...
namespace local_udesbot\output;
...
class widget implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $botname = get_config('local_udesbot', 'botname') ?: get_string('botname_default', 'local_udesbot');
        $widgetlabel = get_config('local_udesbot', 'widgetlabel') ?: get_string('widgetlabel_default', 'local_udesbot');
        $greetingtemplate = get_config('local_udesbot', 'greetingtemplate') ?: get_string('greeting_default', 'local_udesbot');

        // Get v1.9.0 configuration options.
        $inactivitytimeout = get_config('local_udesbot', 'inactivitytimeout') ?: 600000; // 10 minutes default.
        $enablehistory = get_config('local_udesbot', 'enablehistory') ?? 1;

        // Get v2.0.0 configuration options.
        $enablefeedback = get_config('local_udesbot', 'enablefeedback') ?? 1;
        $enablesound = get_config('local_udesbot', 'enablesound') ?? 1;
        ...
        return [
            ...
            'serviceurl' => $CFG->wwwroot . '/local/udesbot/service.php',
            'historyurl' => $CFG->wwwroot . '/local/udesbot/history.php',
            'shortcutsurl' => $CFG->wwwroot . '/local/udesbot/shortcuts_ajax.php',
            'sesskey' => sesskey(),
            'courseid' => $courseid,
            // Widget icon (v1.8.1).
            ...
            'feedbackurl' => $enablefeedback ? $CFG->wwwroot . '/local/udesbot/feedback.php' : '',
    ...
    private function get_current_theme() {
        ...
        $table = new \xmldb_table('local_udesbot_theme');

        if ($dbman->table_exists($table)) {
            // Try to get default theme.
            $theme = $DB->get_record('local_udesbot_theme', ['isdefault' => 1]);
            if ($theme) {
                return $theme;
            }

            // Fallback to first theme.
            $theme = $DB->get_record('local_udesbot_theme', [], '*', IGNORE_MULTIPLE);
            if ($theme) {
                return $theme;
            }
        }
        ...
        $default->primarycolor = get_config('local_udesbot', 'primarycolor') ?: '#0f6fc5';
    ...
    private function prepare_mascot($theme) {
        ...
        switch ($mascottype) {
            case 'clippy':
                $svgpath = $CFG->dirroot . '/local/udesbot/pix/mascots/clippy.svg';
                break;

            case 'robot':
                $svgpath = $CFG->dirroot . '/local/udesbot/pix/mascots/robot.svg';
                break;

            case 'owl':
                $svgpath = $CFG->dirroot . '/local/udesbot/pix/mascots/owl.svg';
                break;

            case 'cat':
                $svgpath = $CFG->dirroot . '/local/udesbot/pix/mascots/cat.svg';
                break;

            case 'lightbulb':
                $svgpath = $CFG->dirroot . '/local/udesbot/pix/mascots/lightbulb.svg';
                break;
