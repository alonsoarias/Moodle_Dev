...
class engine {
    ...
    protected function build_greeting_response(array $analysis): array {
        ...
        if ($hour < 12) {
            $timeGreeting = get_string('greeting_morning', 'local_udesbot');
        } else if ($hour < 19) {
            $timeGreeting = get_string('greeting_afternoon', 'local_udesbot');
        } else {
            $timeGreeting = get_string('greeting_evening', 'local_udesbot');
        }
        ...
        if (!empty($archetypeGreeting)) {
            ...
            $greetings = [
                get_string('greeting_response_1', 'local_udesbot', $params),
                get_string('greeting_response_2', 'local_udesbot', $params),
                get_string('greeting_response_3', 'local_udesbot', $params),
                get_string('greeting_response_4', 'local_udesbot', $params),
            ];
    ...
    protected function build_farewell_response(array $analysis): array {
        ...
        $farewells = [
            get_string('farewell_response_1', 'local_udesbot'),
            get_string('farewell_response_2', 'local_udesbot'),
            get_string('farewell_response_3', 'local_udesbot'),
            get_string('farewell_response_4', 'local_udesbot'),
        ];
    ...
    protected function build_thanks_response(array $analysis): array {
        ...
        $responses = [
            get_string('thanks_response_1', 'local_udesbot'),
            get_string('thanks_response_2', 'local_udesbot'),
            get_string('thanks_response_3', 'local_udesbot'),
            get_string('thanks_response_4', 'local_udesbot'),
        ];
    ...
    protected function build_empathetic_response(array $analysis): ?array {
        ...
        $empathetic = [
            get_string('empathetic_response_1', 'local_udesbot', $userInfo['firstname']),
            get_string('empathetic_response_2', 'local_udesbot'),
            get_string('empathetic_response_3', 'local_udesbot'),
        ];
        ...
        return [
            ...
            'type' => 'empathetic',
            'options' => [
                ['text' => get_string('option_task_problem', 'local_udesbot'), 'icon' => 'bi-file-earmark-text', 'action' => 'problema con mi tarea'],
                ['text' => get_string('option_access_problem', 'local_udesbot'), 'icon' => 'bi-lock', 'action' => 'no puedo acceder'],
                ['text' => get_string('option_contact_teacher', 'local_udesbot'), 'icon' => 'bi-person', 'action' => 'contactar profesor'],
            ],
        ];
    }
    ...
    protected function handle_affirmation(array $analysis): array {
        ...
        return [
            'success' => true,
            'response' => get_string('affirmation_response', 'local_udesbot'),
            'ruleid' => null,
            'confidence' => 1.0,
            'type' => 'follow_up',
    ...
    protected function handle_negation(array $analysis): array {
        return [
            'success' => true,
            'response' => get_string('negation_response', 'local_udesbot'),
            'ruleid' => null,
            'confidence' => 1.0,
            'type' => 'follow_up',
        ];
    ...
    protected function filter_rules_by_context(array $rules, array $userArchetypes, string $userLang): array {
        $autolang = get_config('local_udesbot', 'autolang');
        $filtered = [];
        $seenPatterns = [];
    ...
    protected function build_response(array $match, array $analysis): array {
        ...
        if ($rule->showoptions) {
            ...
            if (empty($options)) {
                $dbOptions = $DB->get_records('local_udesbot_option', [
                    'ruleid' => $rule->id,
                    'enabled' => 1,
                ], 'sortorder ASC');
                ...
                foreach ($dbOptions as $opt) {
                    ...
                    if ($opt->targetruleid) {
                        $targetRule = $DB->get_record('local_udesbot_rule', ['id' => $opt->targetruleid]);
                        if ($targetRule) {
    ...
    protected function handle_no_match(string $question, array $analysis, array $scoredRules): array {
        ...
        $fallbackResponses = [
            get_string('fallback_response_1', 'local_udesbot'),
            get_string('fallback_response_2', 'local_udesbot'),
            get_string('fallback_response_3', 'local_udesbot'),
        ];
        ...
        if (!empty($suggestions)) {
            $response .= "\n\n" . get_string('fallback_suggestions', 'local_udesbot');
    ...
    protected function get_topic_suggestions(string $topic): array {
        $suggestions = [
            intent_detector::TOPIC_ASSIGNMENTS => [
                ['text' => get_string('suggestion_pending_tasks', 'local_udesbot'), 'icon' => 'bi-file-earmark-text', 'action' => 'tareas pendientes'],
                ['text' => get_string('suggestion_next_deadline', 'local_udesbot'), 'icon' => 'bi-calendar', 'action' => 'fecha proxima tarea'],
            ],
            intent_detector::TOPIC_GRADES => [
                ['text' => get_string('suggestion_current_grade', 'local_udesbot'), 'icon' => 'bi-trophy', 'action' => 'mi calificacion'],
                ['text' => get_string('suggestion_my_grades', 'local_udesbot'), 'icon' => 'bi-graph-up', 'action' => 'mis notas'],
            ],
            intent_detector::TOPIC_CALENDAR => [
                ['text' => get_string('suggestion_week_events', 'local_udesbot'), 'icon' => 'bi-calendar-event', 'action' => 'eventos semana'],
                ['text' => get_string('suggestion_next_exam', 'local_udesbot'), 'icon' => 'bi-clock', 'action' => 'proximo examen'],
            ],
            intent_detector::TOPIC_COURSE => [
                ['text' => get_string('suggestion_who_teacher', 'local_udesbot'), 'icon' => 'bi-person', 'action' => 'quien es el profesor'],
                ['text' => get_string('suggestion_course_info', 'local_udesbot'), 'icon' => 'bi-info-circle', 'action' => 'informacion del curso'],
            ],
        ];

        return $suggestions[$topic] ?? [
            ['text' => get_string('suggestion_my_tasks', 'local_udesbot'), 'icon' => 'bi-list-task', 'action' => 'mis tareas'],
            ['text' => get_string('suggestion_my_calendar', 'local_udesbot'), 'icon' => 'bi-calendar3', 'action' => 'mi calendario'],
            ['text' => get_string('suggestion_my_grades_alt', 'local_udesbot'), 'icon' => 'bi-award', 'action' => 'mis calificaciones'],
        ];
    }
    ...
    protected function get_quick_start_options(): array {
        ...
        return [
            ['text' => get_string('option_view_tasks', 'local_udesbot'), 'icon' => 'bi-list-check', 'action' => 'mis tareas pendientes'],
            ['text' => get_string('option_view_grades', 'local_udesbot'), 'icon' => 'bi-trophy', 'action' => 'mis calificaciones'],
            ['text' => get_string('option_view_calendar', 'local_udesbot'), 'icon' => 'bi-calendar-event', 'action' => 'mi calendario'],
        ];
    }
    ...
    protected function log_and_update_context(string $question, array $response, array $analysis, ?int $ruleid): void {
        ...
        try {
            ...
            $DB->insert_record('local_udesbot_log', $log);
        } catch (\Exception $e) {
