...
/**
 * Pattern loader for udesbot - loads NLP patterns from database.
 *
 * This class centralizes pattern loading from the local_udesbot_pattern table.
...
class pattern_loader {
    ...
    public static function load(string $lang = 'es'): void {
        ...
        try {
            ...
            if (!$dbman->table_exists('local_udesbot_pattern')) {
                self::$loaded = true;
                return;
            }

            // Load all patterns for the specified language.
            $patterns = $DB->get_records('local_udesbot_pattern', [
                'lang' => $lang,
                'enabled' => 1
            ], 'type, sortorder');
