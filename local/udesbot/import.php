...
$PAGE->set_url(new moodle_url('/local/udesbot/import.php'));
$PAGE->set_title(get_string('importexport', 'local_udesbot'));
$PAGE->set_heading(get_string('importexport', 'local_udesbot'));

admin_externalpage_setup('local_udesbot_importexport');
...
class import_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('filepicker', 'importfile', get_string('selectfile', 'local_udesbot'),
            null, ['accepted_types' => ['.json', '.csv']]);
        $mform->addRule('importfile', get_string('required'), 'required', null, 'client');

        // Import type options.
        $importtypes = [
            'full' => get_string('import_type_full', 'local_udesbot'),
            'rules_only' => get_string('import_type_rules', 'local_udesbot'),
        ];
        $mform->addElement('select', 'importtype', get_string('import_type', 'local_udesbot'), $importtypes);
        $mform->setDefault('importtype', 'full');

        // Clear existing option.
        $mform->addElement('advcheckbox', 'clearexisting', get_string('clearexisting', 'local_udesbot'));
        $mform->setDefault('clearexisting', 0);
        $mform->addHelpButton('clearexisting', 'clearexisting', 'local_udesbot');

        $this->add_action_buttons(true, get_string('import', 'local_udesbot'));
    }
}
...
function import_rules($importdata, $categorymap, $now) {
    ...
    foreach ($importdata['rules'] as $rule) {
        ...
        if (!empty($rule['categoryid']) && isset($categorymap[$rule['categoryid']])) {
            ...
            $cat = $DB->get_record('local_udesbot_category', ['name' => $rule['category']]);
            if ($cat) {
                $categoryid = $cat->id;
            }
        }
        ...
        $sql = "SELECT * FROM {local_udesbot_rule} WHERE " .
        ...
        if ($existingRule) {
            ...
            $DB->update_record('local_udesbot_rule', $record);
            ...
            $newid = $DB->insert_record('local_udesbot_rule', $record);
...
if ($form->is_cancelled()) {
    redirect(new moodle_url('/local/udesbot/import.php'));
    ...
    if (empty($content)) {
        redirect(new moodle_url('/local/udesbot/import.php'),
            get_string('importerror', 'local_udesbot'),
    ...
    if ($extension === 'csv') {
        ...
        if ($importdata === false) {
            redirect(new moodle_url('/local/udesbot/import.php'),
                get_string('importinvalidcsv', 'local_udesbot'),
                null, \core\output\notification::NOTIFY_ERROR);
        }
    } else {
        // Parse JSON.
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            redirect(new moodle_url('/local/udesbot/import.php'),
                get_string('importinvalidjson', 'local_udesbot'),
                null, \core\output\notification::NOTIFY_ERROR);
        }
    }
    ...
    if (empty($importdata['version'])) {
        redirect(new moodle_url('/local/udesbot/import.php'),
            get_string('importinvalidversion', 'local_udesbot'),
            null, \core\output\notification::NOTIFY_ERROR);
    }

    // Clear existing data if requested.
    if (!empty($data->clearexisting)) {
        $DB->delete_records('local_udesbot_option');
        $DB->delete_records('local_udesbot_rule');
        if ($data->importtype === 'full') {
            $DB->delete_records('local_udesbot_category');
        }
    }
    ...
    if ($data->importtype === 'full' && !empty($importdata['categories'])) {
        ...
        foreach ($importdata['categories'] as $cat) {
            ...
            $existing = $DB->get_record('local_udesbot_category', ['name' => $cat['name']]);
            if ($existing) {
            ...
            $newid = $DB->insert_record('local_udesbot_category', $record);
        ...
        foreach ($importdata['categories'] as $cat) {
            if (!empty($cat['parent']) && isset($categorymap[$cat['parent']])) {
                $DB->set_field('local_udesbot_category', 'parent',
                    $categorymap[$cat['parent']], ['id' => $categorymap[$cat['id']]]);
            }
        }
    } else {
        // Build category map from existing categories.
        $existingCats = $DB->get_records('local_udesbot_category');
    ...
    if (!empty($importdata['options'])) {
        foreach ($importdata['options'] as $opt) {
            ...
            $DB->insert_record('local_udesbot_option', $record);
            $stats['options']++;
        }
    }
    ...
    redirect(new moodle_url('/local/udesbot/import.php'),
        get_string('importsuccess_v2', 'local_udesbot', $stats),
...
// Export section.
echo $OUTPUT->heading(get_string('exportkb', 'local_udesbot'), 3);
echo html_writer::tag('p', get_string('exportkb_desc', 'local_udesbot'));

// Export buttons - JSON and CSV.
$exportjsonurl = new moodle_url('/local/udesbot/export.php', ['format' => 'json', 'sesskey' => sesskey()]);
$exportcsvurl = new moodle_url('/local/udesbot/export.php', ['format' => 'csv', 'sesskey' => sesskey()]);

echo html_writer::start_div('mb-4');
echo html_writer::link($exportjsonurl, get_string('export_json', 'local_udesbot'),
    ['class' => 'btn btn-primary mr-2']);
echo html_writer::link($exportcsvurl, get_string('export_csv', 'local_udesbot'),
    ['class' => 'btn btn-secondary ml-2']);
echo html_writer::end_div();

// Current statistics.
$catcount = $DB->count_records('local_udesbot_category');
$rulecount = $DB->count_records('local_udesbot_rule');
$optcount = $DB->count_records('local_udesbot_option');

echo html_writer::tag('p', get_string('currentstats', 'local_udesbot', [
    'categories' => $catcount,
    'rules' => $rulecount,
    'options' => $optcount,
]));
...
// Import section.
echo $OUTPUT->heading(get_string('importkb', 'local_udesbot'), 3);
echo html_writer::tag('p', get_string('importkb_desc_v2', 'local_udesbot'));

$form->display();

// Template downloads section.
echo html_writer::tag('hr', '');
echo $OUTPUT->heading(get_string('templates', 'local_udesbot'), 3);
echo html_writer::tag('p', get_string('templates_desc', 'local_udesbot'));

$templatejsonurl = new moodle_url('/local/udesbot/db/data/templates/import_template.json');
$templatecsvurl = new moodle_url('/local/udesbot/db/data/templates/import_template.csv');

echo html_writer::start_div('mb-4');
echo html_writer::link($templatejsonurl, get_string('download_json_template', 'local_udesbot'),
    ['class' => 'btn btn-outline-primary mr-2', 'download' => 'import_template.json']);
echo html_writer::link($templatecsvurl, get_string('download_csv_template', 'local_udesbot'),
