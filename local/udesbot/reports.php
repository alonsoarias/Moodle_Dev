...
/**
 * Reports and statistics dashboard for udesbot.
 *
...
admin_externalpage_setup('local_udesbot_reports');
...
$PAGE->set_url('/local/udesbot/reports.php');
$PAGE->set_context($context);
$PAGE->set_title(get_string('reports', 'local_udesbot'));
$PAGE->set_heading(get_string('reports', 'local_udesbot'));

// Add CSS for dashboard cards.
$PAGE->requires->css(new moodle_url('/local/udesbot/styles.css'));
...
echo html_writer::start_tag('div', ['class' => 'udesbot-stats-container mb-4']);

// Get statistics (use unfiltered counts for overview).
$totalconversations = $DB->count_records('local_udesbot_log');
$matchedquestions = $DB->count_records('local_udesbot_log', ['matched' => 1]);
$unmatchedquestions = $DB->count_records('local_udesbot_log', ['matched' => 0]);
$successrate = $totalconversations > 0 ? round(($matchedquestions / $totalconversations) * 100, 1) : 0;
$avgconfidence = $DB->get_field_sql('SELECT AVG(confidence) FROM {local_udesbot_log} WHERE matched = 1');
$avgconfidence = $avgconfidence ? round($avgconfidence * 100, 1) : 0;
$uniqueusers = $DB->count_records_sql('SELECT COUNT(DISTINCT userid) FROM {local_udesbot_log}');

// Cards HTML.
$cards = [
    [
        'title' => get_string('totalconversations', 'local_udesbot'),
        'value' => $totalconversations,
        'class' => 'bg-primary',
        'icon' => 'fa-comments',
    ],
    [
        'title' => get_string('matchedquestions', 'local_udesbot'),
        'value' => $matchedquestions,
        'class' => 'bg-success',
        'icon' => 'fa-check-circle',
    ],
    [
        'title' => get_string('unmatchedquestions', 'local_udesbot'),
        'value' => $unmatchedquestions,
        'class' => 'bg-warning',
        'icon' => 'fa-question-circle',
    ],
    [
        'title' => get_string('successrate', 'local_udesbot'),
        'value' => $successrate . '%',
        'class' => 'bg-info',
        'icon' => 'fa-chart-line',
    ],
    [
        'title' => get_string('averageconfidence', 'local_udesbot'),
        'value' => $avgconfidence . '%',
        'class' => 'bg-secondary',
        'icon' => 'fa-bullseye',
    ],
    [
        'title' => get_string('uniqueusers', 'local_udesbot'),
        'value' => $uniqueusers,
        'class' => 'bg-dark',
        'icon' => 'fa-users',
    ],
...
echo html_writer::tag('h5', get_string('filters', 'local_udesbot'), ['class' => 'mb-0']);
echo html_writer::end_tag('div');
echo html_writer::start_tag('div', ['class' => 'card-body']);

$filterurl = new moodle_url('/local/udesbot/reports.php');
...
echo html_writer::tag('label', get_string('datefrom', 'local_udesbot'), ['class' => 'mr-2']);
...
// Date to.
echo html_writer::start_tag('div', ['class' => 'col-md-3 mb-2']);
echo html_writer::tag('label', get_string('dateto', 'local_udesbot'), ['class' => 'mr-2']);
echo html_writer::empty_tag('input', [
    'type' => 'date',
    'name' => 'dateto_date',
    'class' => 'form-control',
...
echo html_writer::tag('label', get_string('status_header', 'local_udesbot'), ['class' => 'mr-2']);
$matchoptions = [
    '' => get_string('all', 'local_udesbot'),
    'matched' => get_string('matchedonly', 'local_udesbot'),
    'unmatched' => get_string('unmatchedonly', 'local_udesbot'),
];
...
echo html_writer::empty_tag('input', [
    ...
    'class' => 'form-control',
    'value' => $searchquery,
    'placeholder' => get_string('searchquestion', 'local_udesbot'),
]);
echo html_writer::end_tag('div');
...
echo html_writer::start_tag('div', ['class' => 'card mb-4']);
echo html_writer::start_tag('div', ['class' => 'card-header']);
echo html_writer::tag('h5', get_string('recentconversations', 'local_udesbot'), ['class' => 'mb-0']);
echo html_writer::end_tag('div');
echo html_writer::start_tag('div', ['class' => 'card-body']);

// Count total records with filters.
$countsql = "SELECT COUNT(*) FROM {local_udesbot_log} l WHERE $where";
$totalrecords = $DB->count_records_sql($countsql, $params);

if ($totalrecords == 0) {
    echo $OUTPUT->notification(get_string('nologs', 'local_udesbot'), 'info');
} else {
    ...
    $table->head = [
        get_string('user'),
        get_string('question', 'local_udesbot'),
        get_string('status_header', 'local_udesbot'),
        get_string('confidence', 'local_udesbot'),
        get_string('date'),
    ...
    foreach ($logs as $log) {
        ...
        // Status badge.
        if ($log->matched) {
            $status = html_writer::tag('span', get_string('matched', 'local_udesbot'),
                ['class' => 'badge badge-success']);
        } else {
            $status = html_writer::tag('span', get_string('unmatched', 'local_udesbot'),
        ...
        $viewdetails = html_writer::tag('button', get_string('viewdetails', 'local_udesbot'), [
    ...
    // Pagination.
    $baseurl = new moodle_url('/local/udesbot/reports.php', [
        'datefrom' => $datefrom,
        'dateto' => $dateto,
...
echo html_writer::start_tag('div', ['class' => 'card mb-4']);
echo html_writer::start_tag('div', ['class' => 'card-header']);
echo html_writer::tag('h5', get_string('questionswithoutrule', 'local_udesbot'), ['class' => 'mb-0']);
echo html_writer::end_tag('div');
echo html_writer::start_tag('div', ['class' => 'card-body']);

// Get unique unmatched questions with frequency.
$unmatchedsql = "SELECT question, COUNT(*) as frequency
                 FROM {local_udesbot_log}
                 WHERE matched = 0
                 GROUP BY question
...
if (empty($unmatchedquestions)) {
    echo $OUTPUT->notification(get_string('nounmatchedquestions', 'local_udesbot'), 'success');
} else {
    $table = new html_table();
    $table->head = [
        get_string('question', 'local_udesbot'),
        get_string('frequency', 'local_udesbot'),
        get_string('actions_header', 'local_udesbot'),
    ];
    $table->attributes['class'] = 'generaltable table-striped';
    ...
    foreach ($unmatchedquestions as $uq) {
        ...
        $createruleurl = new moodle_url('/local/udesbot/manage.php', [
            'action' => 'add',
            'pattern' => $uq->question,
        ]);
        $createrule = html_writer::link($createruleurl, get_string('createrule', 'local_udesbot'),
...
echo html_writer::tag('h5', get_string('conversationdetails', 'local_udesbot'),
...
echo html_writer::tag('button', html_writer::tag('span', '&times;', ['aria-hidden' => 'true']), [
    ...
    'aria-label' => get_string('close', 'local_udesbot'),
]);
echo html_writer::end_tag('div');
...
echo html_writer::tag('button', get_string('close', 'local_udesbot'), [
    'type' => 'button',
    ...
    document.querySelectorAll('.view-log-details').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            var statusBadge = matched == '1'
                ? '<span class=\"badge badge-success\">" . get_string('matched', 'local_udesbot') . "</span>'
                : '<span class=\"badge badge-warning\">" . get_string('unmatched', 'local_udesbot') . "</span>';
            ...
            var content = '<dl class=\"row\">' +
                ...
                '<dt class=\"col-sm-3\">" . get_string('status_header', 'local_udesbot') . "</dt>' +
                '<dd class=\"col-sm-9\">' + statusBadge + '</dd>' +
                '<dt class=\"col-sm-3\">" . get_string('confidence', 'local_udesbot') . "</dt>' +
                '<dd class=\"col-sm-9\">' + confidence + '</dd>' +
                '<dt class=\"col-sm-3\">" . get_string('question', 'local_udesbot') . "</dt>' +
                '<dd class=\"col-sm-9\"><div class=\"alert alert-info\">' + question + '</div></dd>' +
                '<dt class=\"col-sm-3\">" . get_string('response', 'local_udesbot') . "</dt>' +
            ...
            // Show modal using Bootstrap (compatible with BS4 and BS5).
