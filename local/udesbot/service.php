...
use local_udesbot\bot\context_handler;
use local_udesbot\bot\shortcut_handler;
use local_udesbot\bot\response_builder;
...
if (empty(trim($question)) || strlen($question) > 1000) {
    echo json_encode([
        'success' => false,
        'error' => get_string('invalidquestion', 'local_udesbot'),
    ]);
    exit;
}
...
if ($shortcutresult !== null) {
    ...
    $engine = new \local_udesbot\bot\engine($courseid, $USER->id);
    ...
    if ($result['response'] !== null) {
        ...
        if ($ruleid) {
            $rule = $DB->get_record('local_udesbot_rule', ['id' => $ruleid]);
            ...
            if ($rule && $rule->requiredcontext) {
                ...
                if ($rule->requiredcontext === 'course' && !$contexthandler->is_in_course()) {
                    ...
                    $responsetext = get_string('requirescoursecontext', 'local_udesbot');
                }
            }
        }
        ...
        if ($ruleid) {
            $showoptions = $DB->get_field('local_udesbot_rule', 'showoptions', ['id' => $ruleid]);
            if ($showoptions) {
                $options = $DB->get_records('local_udesbot_option',
                ...
                if ($options) {
                    ...
                    foreach ($options as $option) {
                        if ($option->targetruleid) {
                            $option->targetpattern = $DB->get_field('local_udesbot_rule', 'pattern', ['id' => $option->targetruleid]);
                        }
                    }
        ...
        $response = [
            'success' => true,
            'response' => get_string('noresponse', 'local_udesbot'),
            'ruleid' => null,
            'confidence' => 0,
            'options' => [],
            'type' => 'nomatch',
        ...
        $matched = 0;
        $responsetext = get_string('noresponse', 'local_udesbot');
        $ruleid = null;
        $confidence = 0;
    }
}
