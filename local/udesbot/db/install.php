 ...
 *
 * udesbot v3.2.0 - Complete JSON-based knowledge base architecture.
 *
 ...
 */
function xmldb_local_udesbot_install() {
    global $DB;
    ...
    try {
        ...
        debugging('udesbot install error: ' . $e->getMessage(), DEBUG_DEVELOPER);
        return false;
    }
}
...
function read_json_file($filepath) {
    if (!file_exists($filepath)) {
        throw new moodle_exception('filenotfound', 'local_udesbot', '', $filepath);
    }
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        throw new moodle_exception('jsonparseerror', 'local_udesbot', '', json_last_error_msg());
    }
...
function install_categories_from_json($filepath, $now) {
    ...
    foreach ($data['categories'] as $category) {
        ...
        $catids[$category['id']] = $DB->insert_record('local_udesbot_category', $record);
    }
...
function install_themes_from_json($filepath, $now) {
    ...
    foreach ($data['themes'] as $theme) {
        ...
        $DB->insert_record('local_udesbot_theme', $record);
    }
}
