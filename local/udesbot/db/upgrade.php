...
/**
 * Database upgrade script for local_udesbot.
 *
...
/**
 * Upgrade the local_udesbot plugin.
 *
 * @param int $oldversion The old version of the plugin.
 * @return bool
 */
function xmldb_local_udesbot_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2025121200) {
        ...
        $table = new xmldb_table('local_udesbot_rule');
        ...
        $table = new xmldb_table('local_udesbot_feedback');
        ...
        $table->add_key('ruleid_fk', XMLDB_KEY_FOREIGN, ['ruleid'], 'local_udesbot_rule', ['id']);
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025121200, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025121320) {
        ...
        $themes = $DB->get_records('local_udesbot_theme');
        ...
        foreach ($themes as $theme) {
            ...
            if (empty($theme->mascotenabled) && (empty($theme->mascottype) || $theme->mascottype === 'none')) {
                ...
                $DB->update_record('local_udesbot_theme', $update);
            }
        }

        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025121320, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025121940) {
        ...
        $table = new xmldb_table('local_udesbot_context');
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('userid_fk', XMLDB_KEY_FOREIGN, ['userid'], 'user', ['id']);
        $table->add_key('last_ruleid_fk', XMLDB_KEY_FOREIGN, ['last_ruleid'], 'local_udesbot_rule', ['id']);
        ...
        $ruletable = new xmldb_table('local_udesbot_rule');
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025121940, 'local', 'udesbot');
    }

    // v3.0.1 - Add roles and context fields to shortcuts for role-based filtering.
    if ($oldversion < 2025121942) {

        $table = new xmldb_table('local_udesbot_shortcut');
        ...
        $shortcuts = $DB->get_records('local_udesbot_shortcut');
        foreach ($shortcuts as $shortcut) {
            ...
            $DB->update_record('local_udesbot_shortcut', $update);
        }

        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025121942, 'local', 'udesbot');
    }

    // Version 2025121945: Add action field to options table and reload options.
    if ($oldversion < 2025121945) {
        $table = new xmldb_table('local_udesbot_option');
        ...
        if (file_exists($navfile)) {
            ...
            if ($data && isset($data['rules'])) {
                foreach ($data['rules'] as $rule) {
                    ...
                    $dbrule = $DB->get_record('local_udesbot_rule', ['pattern' => $rule['pattern']]);
                    ...
                    if ($dbrule && isset($rule['options']) && is_array($rule['options'])) {
                        ...
                        foreach ($rule['options'] as $option) {
                            ...
                            $DB->insert_record('local_udesbot_option', $optrecord);
                        }
                    }
                }
            }
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025121945, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025121946) {
        ...
        if (file_exists($navfile)) {
            ...
            if ($data && isset($data['rules'])) {
                ...
                $generalcat = $DB->get_record('local_udesbot_category', ['name' => 'General']);
                ...
                foreach ($data['rules'] as $rule) {
                    ...
                    if ($DB->record_exists('local_udesbot_rule', ['pattern' => $rule['pattern']])) {
                        continue;
                    }
                    ...
                    $ruleid = $DB->insert_record('local_udesbot_rule', $record);
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025121946, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025122100) {
        ...
        $table = new xmldb_table('local_udesbot_pattern');
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025122100, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025122102) {
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025122102, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025122103) {
            ...
            if ($data && isset($data['rules'])) {
                foreach ($data['rules'] as $rule) {
                    ...
                    if ($existingRule) {
                        ...
                        $DB->update_record('local_udesbot_rule', $update);
                    }
                }
            }
        }
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025122103, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025122209) {
        ...
        foreach ($files as $filepath) {
            ...
            foreach ($data['rules'] as $rule) {
                ...
                if ($dbrule && isset($rule['options']) && is_array($rule['options'])) {
                    ...
                    $DB->delete_records('local_udesbot_option', ['ruleid' => $dbrule->id]);
            ...
            if ($data && isset($data['rules'])) {
                foreach ($data['rules'] as $rule) {
                    ...
                    if ($dbrule && isset($rule['options']) && is_array($rule['options'])) {
                        ...
                        $existingOptions = $DB->count_records('local_udesbot_option', ['ruleid' => $dbrule->id]);
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025122209, 'local', 'udesbot');
    }
    ...
    if ($oldversion < 2025122104) {
        ...
        if (!$DB->record_exists('local_udesbot_theme', ['name' => 'Retro'])) {
            $theme = new stdClass();
            $theme->name = 'Retro';
            ...
            $DB->insert_record('local_udesbot_theme', $theme);
        }

        // Drop the schedule table - feature removed.
        $dbman = $DB->get_manager();
        $table = new xmldb_table('local_udesbot_schedule');
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2025122104, 'local', 'udesbot');
    }
...
function load_patterns_from_json($filepath, $type, $now) {
    ...
    foreach ($data[$patternsKey] as $key => $patternData) {
        ...
        $existing = $DB->get_record('local_udesbot_pattern', [
            'type' => $type,
            'patternkey' => $key,
            'lang' => $lang
        ]);
        ...
        if ($existing) {
            ...
            $DB->update_record('local_udesbot_pattern', $record);
        } else {
            $DB->insert_record('local_udesbot_pattern', $record);
        }
    }
}
