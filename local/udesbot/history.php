...
try {
    ...
    switch ($action) {
        case 'get':
            ...
            $total = $DB->count_records('local_udesbot_log', ['userid' => $USER->id]);
        ...
        case 'clear':
            ...
            echo json_encode([
                'success' => true,
                'message' => get_string('historydeleted', 'local_udesbot')
            ]);
            break;
        ...
        case 'delete':
            ...
            $record = $DB->get_record('local_udesbot_log', ['id' => $recordid, 'userid' => $USER->id]);

            if ($record) {
                $DB->delete_records('local_udesbot_log', ['id' => $recordid]);
                echo json_encode([
                    'success' => true,
                    'message' => get_string('recorddeleted', 'local_udesbot')
                ]);
            } else {
                echo json_encode([
                    'success' => false,
                    'error' => get_string('recordnotfound', 'local_udesbot')
                ]);
            }
            break;
