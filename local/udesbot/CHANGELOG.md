...
- **styles.css**: Nuevos estilos para `.udesbot-shortcuts-container` y componentes relacionados
...
- **Documentacion de archivos**: Actualizados headers segun estandar Moodle
  ...
  - Copyright: 2025 OrionCloud<https://orioncloud.com.co>
- **Nombre del plugin**: Cambiado de "Nexo Bot" a "udesbot"
...
- **Cadenas de idioma en JavaScript**: Agregado `strings_for_js()` en lib.php para cargar todas las cadenas necesarias por el widget
  - Corrige error `[[mascot_needmore,local_udesbot]]` y similares que aparecian en la interfaz
...
- **Sistema de feedback**: Botones thumbs up/down en respuestas del bot
  - Persistencia en servidor con tabla `local_udesbot_feedback`
...
- **Tabla local_udesbot_feedback**: Almacena feedback de usuarios por regla
...
- Tabla `local_udesbot_shortcut` para gestion de accesos rapidos
...
- Campo `tags` en tabla `local_udesbot_rule`

## [1.3.0] - 2025-11-29

### Agregado
- **Sistema de categorias jerarquicas**: Organizar reglas en categorias/subcategorias
- Tabla `local_udesbot_category` con soporte para anidamiento
...
- Tabla `local_udesbot_option` para gestion de opciones
...
- Tabla `local_udesbot_log` para registro de conversaciones
...
- Permisos: `local/udesbot:use` y `local/udesbot:manage`
- Tabla `local_udesbot_rule` con campos basicos
