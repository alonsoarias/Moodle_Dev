...
/**
 * Shortcuts management page for educambot.
 *
...
use local_educambot\form\shortcut_form;
...
// Admin setup.
admin_externalpage_setup('local_educambot_shortcuts');

$context = context_system::instance();
require_capability('local/educambot:manage', $context);

$PAGE->set_url(new moodle_url('/local/educambot/shortcuts.php'));
$PAGE->set_title(get_string('manageshortcuts', 'local_educambot'));
$PAGE->set_heading(get_string('manageshortcuts', 'local_educambot'));

$baseurl = new moodle_url('/local/educambot/shortcuts.php');
...
switch ($action) {
    ...
    case 'edit':
        ...
        if ($form->is_cancelled()) {
            ...
            if (!empty($data->id)) {
                $record->id = $data->id;
                $DB->update_record('local_educambot_shortcut', $record);
                \core\notification::success(get_string('shortcutupdated', 'local_educambot'));
            } else {
                $record->timecreated = $now;
                $DB->insert_record('local_educambot_shortcut', $record);
                \core\notification::success(get_string('shortcutcreated', 'local_educambot'));
            }
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading($id ? get_string('editshortcut', 'local_educambot') : get_string('addshortcut', 'local_educambot'));
        $form->display();
        echo $OUTPUT->footer();
        exit;
    ...
    case 'delete':
        ...
        if ($confirm) {
            require_sesskey();
            $DB->delete_records('local_educambot_shortcut', ['id' => $id]);
            \core\notification::success(get_string('shortcutdeleted', 'local_educambot'));
            redirect($baseurl);
        }

        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('deleteshortcut', 'local_educambot'));
        echo $OUTPUT->confirm(
            get_string('confirmdelete', 'local_educambot', $shortcut->name),
    ...
    case 'toggle':
        ...
        $DB->update_record('local_educambot_shortcut', $shortcut);
    ...
    case 'movedown':
        ...
        $shortcut = $DB->get_record('local_educambot_shortcut', ['id' => $id], '*', MUST_EXIST);
        $shortcuts = $DB->get_records('local_educambot_shortcut', [], 'sortorder ASC');
        ...
        $DB->update_record('local_educambot_shortcut', $shortcut);
        $DB->update_record('local_educambot_shortcut', $swapshortcut);
    ...
    default:
        // List shortcuts.
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manageshortcuts', 'local_educambot'));

        // Add shortcut button.
        $addurl = new moodle_url($baseurl, ['action' => 'add']);
        echo html_writer::div(
            $OUTPUT->single_button($addurl, get_string('addshortcut', 'local_educambot'), 'get'),
            'mb-3'
        );
        ...
        if (empty($shortcuts)) {
            echo $OUTPUT->notification(get_string('noshortcuts', 'local_educambot'), 'info');
        } else {
            $table = new html_table();
            $table->head = [
                get_string('icon', 'local_educambot'),
                get_string('shortcutname', 'local_educambot'),
                get_string('actiontype', 'local_educambot'),
                get_string('keywords', 'local_educambot'),
        ...
        // Help section.
        echo html_writer::tag('h4', get_string('shortcutshelp', 'local_educambot'), ['class' => 'mt-4']);
        echo html_writer::tag('p', get_string('shortcutshelp_desc', 'local_educambot'));

        // Action types help.
        echo html_writer::tag('h5', get_string('availableactiontypes', 'local_educambot'));
        $actionlist = '<ul>';
