...
/**
 * Conversation context for educambot - manages conversational state.
 *
 * Features:
...
class conversation_context {
    ...
    private function load_from_db(): bool {
        ...
        try {
            $record = $DB->get_record('local_educambot_context', [
                'userid' => $this->userid,
    ...
    public function save_to_db(): void {
        ...
        try {
            ...
            if ($record) {
                ...
                $DB->update_record('local_educambot_context', $record);
            } else {
                $record = new \stdClass();
                ...
                $record->timemodified = time();
                $DB->insert_record('local_educambot_context', $record);
            }
        } catch (\Exception $e) {
    ...
    public function reset(): void {
        ...
        try {
            $DB->delete_records('local_educambot_context', [
                'userid' => $this->userid,
                'sessionid' => $this->sessionid,
            ]);
        } catch (\Exception $e) {
